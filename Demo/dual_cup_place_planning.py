...
def predict_place_from_queue(
    ...
    valid_refs: List[np.ndarray] = []
    for uv in uv_ints:
        ref = pixel_to_ref_point_safe(
            uv,
            depth,
            robot_part=arm,
        )
        if ref is None:
            print(f"预测像素 {uv} 深度无效或像素越界，跳过该点")
            continue
        valid_uvs.append(uv)
        ...
        while cycle_idx < total_cycles:
            ...
            if need_pick:
                if current_pick_ref is None:
                    arx.step_lift(13.5)
                    pick_color, pick_depth = get_aligned_frames(
                    ...
                    current_pick_ref = pixel_to_ref_point_safe(
                        current_pick_uv,
                        pick_depth,
                        robot_part=arm,
                    )
                    if current_pick_ref is None:
                        print(
                            f"预测像素 {current_pick_uv} 深度无效或像素越界，自动刷新"
                        )
                        current_pick_uv = None
                        current_pick_ref = None
                        continue
            ...
            if need_place:
                if place_ref is None:
                    ...
                    if place_height > 13.5:
                        time.sleep(1.0)
                    place_color, place_depth = get_aligned_frames(
                        arx, depth_median_n=depth_median_n)
                    ...
                    if manual:
                        ...
                        if place_idx < 3:
                            ...
                            if place_ref is None:
                                print(
                                    f"预测像素 {predicted_place_uv} 深度无效或像素越界，自动刷新"
                                )
                                predicted_place_uv = None
                                place_ref = None
                                continue
            ...
            if display_color is None:
                display_color, _ = get_aligned_frames(
                    arx, depth_median_n=depth_median_n)
                if display_color is None:
                    continue
